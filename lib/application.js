...
/**
 * 算是一个核心的分发程序，将前端发送的请求通过 handle 在这里进行分发判断。
 ...
 * @private
 * @param {*} callback 一般来说callback 是不存在的，如果通过 app.use 方法传递的是一个 app ，
 *  那么此时的 callback 就会带有参数
 */
app.handle = function handle(req, res, callback) {
  var router = this._router;
  // final handler
  // done 是一个错误回调 404 或者 500
  // finalhandler 传递 req，res，然后返回一个 done 方法。判断为 404 还是 500
...
/**
 * 调用 this._router.use 方法 , _router 是 new Router 的实例
 ...
 * 目前来说，应该是因为这个 createServer 有一个 handle 方法可以做到对每个请求都做处理
 * 发现并不是 createServer 触发了 handle 而是，其他地方触发的
 * 
 * createServer 支持参数传递一个function，这样就可以做到每次的请求都可以做到调用 callback
